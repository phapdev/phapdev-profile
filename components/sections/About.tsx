...
const categories: Skill['category'][] = ['Frontend', 'Backend', 'DevOps', 'Tools', 'Web3'];
...
const SkillCard: React.FC<{ skill: Skill }> = ({ skill }) => (
  <motion.div
    ...
    whileHover={{ scale: 1.1, zIndex: 10, boxShadow: '0 0 15px rgba(0, 255, 255, 0.5)' }}
...
export const AboutPage: React.FC = () => {
  return (
    <div className="h-full flex flex-col p-4 md:p-8 overflow-y-auto">
      <h1 className="text-4xl md:text-5xl font-bold text-glow mb-4">Personal File</h1>
      <p className="text-accent/80 max-w-4xl text-accent md:text-lg mb-8">
        I am phapdev.</p>

      <h2 className="text-3xl md:text-4xl font-bold text-glow mb-6">Skill Tree</h2>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {categories.map(category => (
          <div key={category}>
            <h3 className="text-xl md:text-2xl font-semibold text-primary mb-4">{category}</h3>
            <motion.div
